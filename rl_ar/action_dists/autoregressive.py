...
class AutoregressiveActionDistribution(TorchDistributionWrapper):
    ...
    def deterministic_sample(self) -> TensorType:
        ...
        action_b = self._action_b_dist(action_a).deterministic_sample()
    ...
    def sample(self) -> TensorType:
        ...
        action_b = self._action_b_dist(action_a).sample()
    ...
    def logp(self, actions: TensorType) -> TensorType:
        ...
        action_logp = self._action_a_dist().logp(action_a) + self._action_b_dist(
            action_a
        ).logp(action_b)
        return action_logp
    ...
    def multi_entropy(self) -> TensorType:
        a_dist = self._action_a_dist()
        entropy_a = a_dist.entropy()
        entropy_b = self._action_b_dist(a_dist.sample()).entropy()
    ...
    @override(ActionDistribution)
    def multi_kl(self, other: AutoregressiveActionDistribution) -> TensorType:
        a_dist = self._action_a_dist()
        action_a = a_dist.sample()
        kl_a = a_dist.kl(other._action_a_dist())
        kl_b = self._action_b_dist(action_a).kl(other._action_b_dist(action_a))
    ...
    def _action_b_dist(self, action_a: torch.Tensor) -> TorchCategorical:
        logits_b = self.model.forward_action_b(self.inputs, action_a)
